...
pyr1 = gaussian_pyramid(zeros(r,c,1));
nlev = length(pyr1);  
pyr = gaussian_pyramid(zeros(r,c,1), nlev-2);

% multiresolution blending
for i = 1:N
    % construct pyramid from each input image
	pyrW = gaussian_pyramid(W(:,:,i),nlev-2);
	pyrI = laplacian_pyramid(I(:,:,1,i),nlev-2); % 不用滤波的情况下，减少高层曝光去halo的形成
    % blend level 1:n-3
    for l = 1:nlev-3
...
% saliency measure
function C = saliency(I)
for i = 1:N
    yuv(:,:,:,i) = imfilter(I(:,:,:,i), fspecial('gaussian', 3, 3), 'symmetric', 'conv');
    y = double(yuv(:,:,1,i)); ym = mean(mean(y));
    u = double(yuv(:,:,2,i)); um = mean(mean(u));
    v = double(yuv(:,:,3,i)); vm = mean(mean(v));
    C(:,:,i) = (y-ym).^2 + (u-um).^2 + (v-vm).^2;
end
end
